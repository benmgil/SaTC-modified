...
def taint_stain_analysis(binary, ghidra_analysis_result, output):
    ...
    elif "basic_taint" in ghidra_analysis_result:
        analysis_type = "basictaint"
    ...
    for i in range(0, len(cont) / 3):
        ...
        try:
            ...
            #taint_addr = [string addr?, address of func that refs string][0]
            #func_addr = [addresses of calls to the next func]
            #sinkTargets = [addresses of call to sink funcs]
            ...
            if proj.arch.name != "MIPS32":
                ...
                if not proj.loader.main_object.pic:
                    print("NOT PIC")
                    print("func addr", [hex(j) for j in func_addr])
                    print("taint addr", hex(taint_addr))
                    print("sink targets", [hex(j) for j in sinkTargets])
                    print("cont", cont[i*3])
                    print("cont split", cont[i*3].split(' ')[1])
                    print("cfgnode", cfg.get_any_node(int(cont[i * 3].split(' ')[1], 0),
                                                  anyaddr=True))
                    start_addr = cfg.get_any_node(int(cont[i * 3].split(' ')[1], 0), anyaddr=True).addr
                    callerbb = None
                else:
                    ...
                    print(sinkTargets)
                    print(cont[i*3])
                    print(cont[i*3].split(' ')[1])
                    print(cfg.get_any_node(int(cont[i * 3].split(' ')[1], 0) - 0x10000 + 0x400000,
                                                  anyaddr=True))
            ...
            sinkTargets = [cfg.get_any_node(j, anyaddr=True).addr for j in sinkTargets]
            # print("new sink targets", [j.addr for j in sinkTargets])
            print("new sink targets", sinkTargets)
            print("HOYA")
            for j in func_addr:
                print(j)
                print(cfg.get_any_node(j, anyaddr=True).addr)
            ...
            with open('progress.txt', 'a') as f:
                print(f)
                f.write("%s - %d of %d Analyzing %s from 0x%X, taint 0x%X, sinkTarget%s, functrace %s\n" % (datetime.datetime.now(), i, len(cont)/3,
                binary, start_addr, taint_addr, str([hex(j) for j in sinkTargets]), str([hex(j) for j in followtar])))
            if not callerbb:
                main(start_addr, taint_addr, binary, proj, cfg)
            else:
