...
class CoreTaint:
    ...
    def __init__(self, p, interfunction_level=0, log_path='/tmp/coretaint.out', exploration_strategy=None, smart_call=True, follow_unsat=False, try_thumb=False, white_calls=[], black_calls=[], not_follow_any_calls=False, default_log=True, exit_on_decode_error=True, concretization_strategy=None, force_paths=False, reverse_sat=False, only_tracker=False, shuffle_sat=False, taint_returns_unfollowed_calls=False, taint_arguments_unfollowed_calls=False, allow_untaint=True, logger_obj=None):
        """
        ...
        self._concretization_strategy = self._default_concretization_strategy if concretization_strategy is None else concretization_strategy
    ...
    def _set_deref_bounds(self, ast_node):
        ...
        if hasattr(ast_node, 'op') and ast_node.op == 'Extract' and self.is_tainted(ast_node.args[2]):
    ...
    def is_or_points_to_tainted_data(self, x, path, unconstrained=False):
        ...
        :return:
        """
        return self.is_tainted(x, path=path, unconstrained=unconstrained) or self.is_tainted(self.safe_load(path, x, unconstrained=unconstrained), path=path, unconstrained=unconstrained)
    ...
    def _flat_explore(self, current_path, check_path_fun, guards_info, current_depth, **kwargs):
        ...
        try:
            ...
            print("'Function check path errored out: %s" % str(e))
        ...
        if succ_path.deadended and not succ_states_sat and not succ_states_unsat:
            ...
            print("Backtracking from dead path")
            return
        ...
        for next_state in succ_states:
            ...
            if hasattr(next_state.ip, 'symbolic') and next_state.ip.symbolic:
                if next_state.sat:
                    ...
                    print("Next state UNSAT")
                log.warning("Got a symbolic IP, perhaps a non-handled switch statement? FIX ME... ")
                print("Got a symbolic IP, perhaps a non-handled switch statement? FIX ME... ")
                continue
            ...
            try:
                print("first, lets see if we can follow the calls")
                    ...
                    if not self._is_summarized(current_path, next_path, current_depth):
                        ...
                        if not self._follow_call(current_path, next_path, current_depth):
                            ...
                            if self.get_addr(next_path) not in self._p.loader.main_object.plt.values() and getBugFindingFlag() and self.get_addr(current_path) not in followTarget:
                                ...
                                if 'memset' in self._p.loader.main_object.plt.keys() and self.get_addr(next_path)==self._p.loader.main_object.plt['memset']:
            ...
            # we have a back jump
            if self.get_state(next_path).history.jumpkind == 'Ijk_Boring' and self.get_addr(next_path) <= self.get_addr(current_path) and not self._follow_back_jump(current_path, next_path, guards_info):
            ...
            print("Back to block %s", hex(self.get_addr(current_path)))
            self._new_path = True
    ...
    def run(self, state, sinks_info, sources_info, summarized_f=None, init_bss=True, check_func=None, force_thumb=False, use_smart_concretization=True):
        """
