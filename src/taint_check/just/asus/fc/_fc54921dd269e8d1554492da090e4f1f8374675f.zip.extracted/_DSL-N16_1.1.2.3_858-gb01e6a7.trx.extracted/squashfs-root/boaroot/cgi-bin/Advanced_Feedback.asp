...
function applyRule(){		
		...
		var re_valid = 0;
		if(document.adv_adsl.fb_serviceno.value != ""){
			if(!re_asus.test(document.adv_adsl.fb_serviceno.value)){
				re_valid++;             
			}
			if(document.adv_adsl.fb_serviceno.value.length != 5 || !re_crs.test(document.adv_adsl.fb_serviceno.value)){
				re_valid++;             
			}
