...
heuristicMin = 0
sinks = ['strcpy']
shareSinks = ['nvram_safe_set', 'nvram_bufset', 'setenv']
...
def parseShare(shares):
    nameToFunc = {}
    shareSearchResult = defaultdict(set)
    for funcName in shares:
        for key in shares[funcName]:
            replacedName = str(funcName).replace('set', 'get')
            if replacedName in nameToFunc:
                func = nameToFunc[replacedName]
            else:
                func = getFunction(replacedName)
                if func is None:
                    for f in currentProgram.functionManager.getFunctions(True):
                        if f.name == replacedName:
                            func = f
                            break
                nameToFunc[replacedName] = func
            if func:
                shareSearchResult[func].add(key)
...
def findSinkPath(refaddr, stringaddr, stringval, shareFunc=None):
    pending = []
    #create call graph from func containing ref
    #then recurse through and find if path to sink exists
    #if vulnerable call searchStrArg and then return vulnerable

    def search(func, start=None):
        #for the func that contains a given ref, find all functions it calls
        #callMap[func][address of call] = address of called function
        if func in callMap:
            return
        callMap[func] = {}
    ...
    def printpath(path, isShare):
        if shareFunc is None:
    ...
    def dfs(func, path, start=None):
        #recurse through all callees of func and their callees
        #if it's a sink and format/constantstr is checked correctly then return true
        #otherwise continue down adding to path, skipping function if the format/conststr conditions dont apply
        #if a func has been checked already and its safe then skip too
        ...
        if func.name in sinks and len(path):
            ...
            return True
        if func.name in shareSinks and len(path):
    ...
    if vulnerable and shareFunc is None:
        searchStrArg(startFunc)
    return vulnerable

def searchShareFunc(shareSearchResult):
    for func in shareSearchResult:
        for ref in getReferencesTo(func.entryPoint):
            ...
            if regValue:
                ...
                key = getStr(strAddr)
                if key and key in shareSearchResult[func]:
                    print("SINKING")
                    findSinkPath(ref.fromAddress, strAddr, key, func)


def searchParam(target, refstart=None, refend=None):
    #for a given target param (keyword?)
    #loops through each instance of param
    #   loops through each ref to each instance of param (refstart <= ref <= refend)
    #       get caller (function containing ref.fromAddress) (or function containing function containing)
    #       if we find a caller at either level
    #           add ref.fromAddress to checkedRefs
    #           haveWayToSink = haveWayToSink || findSinkPath(ref.fromAddress, current instance of target, target)
    if DEBUG:
...
if __name__ == '__main__':
    ...
    paramTargets = [args[0]]
    if paramTargets[0] == "None":
        print("Error: No param given to analyze")
        exit(-1)
    f = None
    if len(args) > 1:
        f = open(args[1], 'w')
    ...
    print("Shares to sink:")
    if f is not None:
        print >>f, "Shares to sink:"

    shareSearchResult = parseShare(shareResult)
    # for func in shareSearchResult:
    searchShareFunc(shareSearchResult)
